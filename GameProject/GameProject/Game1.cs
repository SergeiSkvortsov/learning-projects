...
namespace GameProject
{
    ...
    public class Game1 : Game
    {
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            for (int i = 0; i < bears.Count; i++)
            {
                for (int j = i + 1; j < bears.Count; j++)
                {
                    ...
                    {
                        CollisionResolutionInfo x = CollisionUtils.CheckCollision(gameTime.ElapsedGameTime.Milliseconds,
                        ...
                        if (x != null)
                        {
                            if (x.FirstOutOfBounds)
                            {
                                bears[i].Active = false;
                            }
                            else
                            {
                                bears[i].Velocity = x.FirstVelocity;
                                bears[i].DrawRectangle = x.FirstDrawRectangle;
                            }
                            if (x.SecondOutOfBounds)
                            {
                                bears[j].Active = false;
                            }
                            else
                            {
                                bears[j].Velocity = x.SecondVelocity;
                                bears[j].DrawRectangle = x.SecondDrawRectangle;
                            }
                            teddyBounce.Play();
                        }
                    }
            ...
            foreach (TeddyBear bear in bears)
            {
                if (bear.Active)
                {
                    if (bear.CollisionRectangle.Intersects(burger.CollisionRectangle))
                    {
                        ...
                        bear.Active = false;
                        explosions.Add(new Explosion(explosionSpriteStrip, bear.CollisionRectangle.X, bear.CollisionRectangle.Y, explosion));
                        healthString = GameConstants.HealthPrefix + burger.Health;
                        burgerDamage.Play();
                        CheckBurgerKill();
                    }
        ...
        private void SpawnBear()
        {
            ...
            // make sure don't spawn into a collision
